...
.then(resultOfBcrypt=>{
   if(!resultOfBcrypt)
   {
    ...
    const token=jwt.sign({email:loadUser.email,name:loadUser.name,userId:loadUser._id.toString()},'somesupersecretsecret',  {expiresIn:'1h'})

             // console.log("this is token for this user \n:"+token +"\n loadUser._id.toString():\n"+ loadUser._id.toString())

         res.status(200).json({token:token,userId:loadUser._id.toString(),role:"admin",status:true})
})
.catch((err)=>{if(!err.statusCode){err.statusCode =500;}next(err)})
